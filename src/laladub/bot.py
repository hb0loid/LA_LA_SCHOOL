...
async def _prepare_input_audio_as_video(
    ...
    source_roots = _audio_visual_source_roots(settings)
...
def _audio_visual_source_roots(settings: BotSettings) -> list[Path]:
...
async def _prepare_job_media_before_enqueue(
    status_message: Any,
    settings: BotSettings,
    user_id: int | None,
    job: dict[str, Any],
    ...
    if job.get("input_source") == "telegram_audio":
        if await asyncio.to_thread(has_video_and_audio, input_path):
            return True
        job.setdefault("audio_input_path", str(input_path))
        existing_visual = job_dir / "input_audio_visual.mp4"
        if existing_visual.exists() and await asyncio.to_thread(has_video_and_audio, existing_visual):
            job["input_path"] = str(existing_visual)
            _save_job_snapshot(job_dir, job, status="queued", audio_visual=True)
            return True
        input_path = await _prepare_input_audio_as_video(status_message, settings, user_id, job_dir, input_path)
        if input_path is None:
    ...
    if job.get("visual_mode") == "random":
        if (
            (job.get("random_visual") or "_fun_visual" in input_path.stem)
            and await asyncio.to_thread(has_video_and_audio, input_path)
        ):
            job["random_visual"] = True
...
async def _enqueue_job(update: Any, context: Any, job: dict[str, Any], status_message: Any) -> None:
    ...
    prepared = await _prepare_job_media_before_enqueue(status_message, settings, user.id if user else None, job)
    if not prepared:
        return
...
async def _recover_interrupted_jobs(application: Any) -> None:
    ...
    for job in jobs:
        ...
        prepared = await _prepare_job_media_before_enqueue(status_message, settings, user_id, job)
        if not prepared:
            skipped += 1
            continue
