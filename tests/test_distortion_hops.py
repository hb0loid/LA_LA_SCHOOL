...
class UnparseablePivotTests(unittest.TestCase):
    def test_malay_is_allowed_again(self) -> None:
        # It used to be dropped: Argos could not read it, so the chain only
        # worked while the online translator did. MiniSBD chunking reads it
        # locally now, and it is the variant people asked to keep.
        chains = _translation_distortion_chains(_config(9, "en,ms,he,en"))
        self.assertEqual(chains, [["ru", "en", "ms", "he", "en", "ru"]])

    def test_a_listed_language_is_still_dropped(self) -> None:
        # The filter itself stays, for whatever breaks next.
        with patch.object(pipeline, "UNPARSEABLE_PIVOT_LANGS", frozenset({"ms"})):
            self.assertEqual(_translation_distortion_chains(_config(9, "en,ms,he,en")), [])
    ...
    def test_dubbing_into_a_listed_language_distorts_nothing(self) -> None:
        # A chain starts at the target and comes back to it, so a listed
        # target means reading it on the first hop. Leaving the translation
