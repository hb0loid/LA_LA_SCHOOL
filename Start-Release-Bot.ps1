...
$env:LALADUB_EXECUTOR_MODE = "local"
$env:LALADUB_MAX_LOCAL_JOBS = "1"
$env:LALADUB_WORKER_API_HOST = "0.0.0.0"
$env:LALADUB_WORKER_API_PORT = "8765"
$trustedWorkerIps = [Environment]::GetEnvironmentVariable("LALADUB_WORKER_API_TRUSTED_IPS", "User")
if ($trustedWorkerIps) { $env:LALADUB_WORKER_API_TRUSTED_IPS = $trustedWorkerIps }
...
$watermarkImage = [Environment]::GetEnvironmentVariable("LALADUB_WATERMARK_IMAGE", "User")
$env:LALADUB_WATERMARK_IMAGE = if ($watermarkImage) { $watermarkImage } else { Join-Path $Root "assets" }
